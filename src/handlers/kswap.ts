...
export const tvl: Handler = async (_, context) => {
  ...
  const text = Number.parseFloat(result.kalyswapFactories[0].totalLiquidityUSD).toFixed(2);
...
export const volume: Handler = async (_, context) => {
  ...
  const text = Number.parseFloat(result.kalyswapFactories[0].totalVolumeUSD).toFixed(2);
...
export const supply: Handler = async (_, context) => {
  ...
  if (chainInfo.chainId === '3888') {
    // Override Avalanche total supply to account for TreasuryVesterProxy logical burns
    text = ONE_TOKEN.mul(538_000_000).toString();
  } else {
    const totalSupply = await getTotalSupply(chainInfo.rpc, chainInfo.kswap);
...
export const supplyWhole: Handler = async (_, context) => {
  ...
  if (chainInfo.chainId === '3888') {
    // Override Avalanche total supply to account for TreasuryVesterProxy logical burns
    text = BigNumber.from(538_000_000).toString();
  } else {
    const totalSupply = await getTotalSupply(chainInfo.rpc, chainInfo.kswap);
...
export const circulating: Handler = async (_, context) => {
  ...
  if (chainInfo.chainId === '3888') {
    ...
    text = ONE_TOKEN.mul(538_000_000)
      .sub(await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.treasury_vester))
      .sub(await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.community_treasury))
      .toString();
  } else {
    text = (await getTotalSupply(chainInfo.rpc, chainInfo.kswap))
      .sub(await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.treasury_vester))
      .sub(await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.community_treasury))
      .toString();
  }
...
export const treasury: Handler = async (_, context) => {
  ...
  const balance = await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.community_treasury);
...
export const treasuryWhole: Handler = async (_, context) => {
  ...
  const text = (await getBalance(chainInfo.rpc, chainInfo.kswap, chainInfo.community_treasury))
    .div(ONE_TOKEN)
    .toString();
